...
Token* readIdentKeyword(void) {
  ...
  token->tokenType = checkKeyword(token->string); //khi gap ki tu ket thuc thi kiem tra xem no phai keyword hay khong
...
Token* readNumber(void) {
  Token *token = makeToken(TK_NUMBER, lineNo, colNo); //tao token
  int count = 0;
...
Token* getToken(void) {
  ...
  case CHAR_PLUS:
    token = makeToken(SB_PLUS, lineNo, colNo); // phat hien ki tu +
    readChar();
    return token;
  case CHAR_MINUS:
    token = makeToken(SB_MINUS, lineNo, colNo); // phat hien ki tu  -
    readChar();
    return token;
